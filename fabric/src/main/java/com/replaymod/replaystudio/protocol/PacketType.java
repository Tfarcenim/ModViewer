...
public enum PacketType {
   ...
   Tags(ProtocolVersion.v1_14, 91),
   PlayerActionAck(ProtocolVersion.v1_14, 92),
   CombatEnd(ProtocolVersion.v1_17, 51),
   CombatEnter(ProtocolVersion.v1_17, 52),
   CombatEntityDead(ProtocolVersion.v1_17, 53),
   DestroyEntity(ProtocolVersion.v1_17, 58),
   UpdateSimulationDistance(ProtocolVersion.v1_18, 87),
   Features(ProtocolVersion.v1_19_3, 103),
   PlayerListEntryRemove(ProtocolVersion.v1_19_3, 53),
   Bundle(ProtocolVersion.v1_19_4, 0);

   private final State state;
   private final ProtocolVersion initialVersion;
   private final int initialId;

   private PacketType(ProtocolVersion initialVersion, int initialId) {
      this(initialVersion, initialId, State.PLAY);
   }

   private PacketType(ProtocolVersion initialVersion, int initialId, State state) {
      this.state = state;
