...
public class CubicSplineInterpolator extends PolynomialSplineInterpolator {
   ...
   protected void fillMatrix(double[][] matrix, double[] xs, double[] ys, int num, InterpolationParameters params) {
      ...
      for(int i = 0; i < num; ++i) {
         ...
         if (i < num - 1) {
            x = xs[i + 1];
            matrix[row][i * 4] = -(matrix[row][i * 4 + 4] = 3.0D * x * x);
            matrix[row][i * 4 + 1] = -(matrix[row][i * 4 + 5] = 2.0D * x);
            matrix[row][i * 4 + 2] = -(matrix[row][i * 4 + 6] = 1.0D);
            ++row;
         }

         if (i < num - 1) {
            x = xs[i + 1];
            matrix[row][i * 4] = -(matrix[row][i * 4 + 4] = 6.0D * x);
            matrix[row][i * 4 + 1] = -(matrix[row][i * 4 + 5] = 2.0D);
            ++row;
         }
      }
