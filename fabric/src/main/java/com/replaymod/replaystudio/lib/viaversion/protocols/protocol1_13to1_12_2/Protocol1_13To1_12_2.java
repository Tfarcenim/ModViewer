...
public class Protocol1_13To1_12_2 extends AbstractProtocol<ClientboundPackets1_12_1, ClientboundPackets1_13, ServerboundPackets1_12_1, ServerboundPackets1_13> {
   ...
   private static final Set<Character> FORMATTING_CODES = Sets.newHashSet((Object[])('k', 'l', 'm', 'n', 'o', 'r'));
      ...
      this.registerClientbound(ClientboundPackets1_12_1.UNLOCK_RECIPES, new PacketHandlers() {
         public void register() {
            ...
            this.handler((wrapper) -> {
               ...
               if (action == 0) {
                  wrapper.create(ClientboundPackets1_13.DECLARE_RECIPES, (PacketHandler)((wrapper1) -> {
                     wrapper1.write(Type.VAR_INT, RecipeData.recipes.size());
                     Iterator var1 = RecipeData.recipes.entrySet().iterator();

                     while(true) {
                        while(var1.hasNext()) {
                           Entry<String, RecipeData.Recipe> entry = (Entry)var1.next();
                           wrapper1.write(Type.STRING, entry.getKey());
                           wrapper1.write(Type.STRING, ((RecipeData.Recipe)entry.getValue()).getType());
                           String var3 = ((RecipeData.Recipe)entry.getValue()).getType();
                           byte var4 = -1;
                           switch(var3.hashCode()) {
                           case -571676035:
                              if (var3.equals("crafting_shapeless")) {
                                 var4 = 0;
                              }
                              break;
                           case -491776273:
                              if (var3.equals("smelting")) {
                                 var4 = 2;
                              }
                              break;
                           case 1533084160:
                              if (var3.equals("crafting_shaped")) {
                                 var4 = 1;
                              }
                           }
                           ...
                           switch(var4) {
                           case 0:
                              wrapper1.write(Type.STRING, ((RecipeData.Recipe)entry.getValue()).getGroup());
                              wrapper1.write(Type.VAR_INT, ((RecipeData.Recipe)entry.getValue()).getIngredients().length);
                              var11 = ((RecipeData.Recipe)entry.getValue()).getIngredients();
                              i = var11.length;

                              for(var7 = 0; var7 < i; ++var7) {
                                 ingredient = var11[var7];
                                 clonex = (Item[])ingredient.clone();

                                 for(ix = 0; ix < clonex.length; ++ix) {
                                    if (clonex[ix] != null) {
                                       clonex[ix] = new DataItem(clonex[ix]);
                                    }
                                 }

                                 wrapper1.write(Type.FLAT_ITEM_ARRAY_VAR_INT, clonex);
                              }

                              wrapper1.write(Type.FLAT_ITEM, new DataItem(((RecipeData.Recipe)entry.getValue()).getResult()));
                              break;
                           case 1:
                              wrapper1.write(Type.VAR_INT, ((RecipeData.Recipe)entry.getValue()).getWidth());
                              wrapper1.write(Type.VAR_INT, ((RecipeData.Recipe)entry.getValue()).getHeight());
                              wrapper1.write(Type.STRING, ((RecipeData.Recipe)entry.getValue()).getGroup());
                              var11 = ((RecipeData.Recipe)entry.getValue()).getIngredients();
                              i = var11.length;
                           ...
                           case 2:
                              ...
                              wrapper1.write(Type.FLAT_ITEM, new DataItem(((RecipeData.Recipe)entry.getValue()).getResult()));
                              wrapper1.write(Type.FLOAT, ((RecipeData.Recipe)entry.getValue()).getExperience());
                              wrapper1.write(Type.VAR_INT, ((RecipeData.Recipe)entry.getValue()).getCookingTime());
                           }
                        }
