...
public final class PaletteType1_18 extends Type<DataPalette> {
   ...
   public DataPalette read(ByteBuf buffer) throws Exception {
      ...
      if (originalBitsPerValue == 0) {
         ...
         if (originalBitsPerValue >= 0 && originalBitsPerValue <= this.type.highestBitsPerValue()) {
            if (this.type == PaletteType.BLOCKS && originalBitsPerValue < 4) {
               bitsPerValue = 4;
            }
         } else {
