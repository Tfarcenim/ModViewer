...
public class VideoRenderer implements RenderInfo {
   ...
   private void finish() {
      ...
      while(var1.hasNext()) {
         ...
         this.mc.options.getSoundSourceOptionInstance((SoundSource)entry.getKey()).set((double)(Float)entry.getValue());
      }
   ...
   private void executeTaskQueue() {
      while(true) {
         if (this.mc.getOverlay() != null) {
            ...
            ((MinecraftAccessor)this.mc).setResourceReloadFuture((CompletableFuture)null);
            this.mc.reloadResourcePacks().thenRun(() -> {
               resourceReloadFuture.complete((Object)null);
            });
         }
      }
   }
   ...
   public static String[] checkCompat(Stream<RenderSettings> settings) {
      return (String[])settings.map(VideoRenderer::checkCompat).filter(Objects::nonNull).findFirst().orElse((Object)null);
   }
