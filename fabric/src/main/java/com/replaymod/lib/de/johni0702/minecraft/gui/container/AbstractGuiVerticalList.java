...
public abstract class AbstractGuiVerticalList<T extends AbstractGuiVerticalList<T>> extends AbstractGuiScrollable<T> implements Draggable {
   ...
   public AbstractGuiVerticalList() {
      this.listPanel = (GuiPanel)(new GuiPanel(this)).setLayout(this.listLayout);
      this.setLayout(new CustomLayout<T>() {
         protected void layout(T container, int width, int height) {
            this.pos(AbstractGuiVerticalList.this.listPanel, width / 2 - this.width(AbstractGuiVerticalList.this.listPanel) / 2, 5);
         }

         public ReadableDimension calcMinSize(GuiContainer<?> container) {
            ReadableDimension panelSize = AbstractGuiVerticalList.this.listPanel.getMinSize();
            return new ReadableDimension(panelSize) {
               // $FF: synthetic field
               final ReadableDimension val$panelSize;
               // $FF: synthetic field
               final <undefinedtype> this$1;

               {
                  this.this$1 = this$1;
                  this.val$panelSize = var2;
               }

               public int getWidth() {
                  return this.val$panelSize.getWidth();
               }

               public int getHeight() {
                  return this.val$panelSize.getHeight() + 10;
               }
   ...
   public T setDrawSlider(boolean drawSlider) {
      ...
      return (AbstractGuiVerticalList)this.getThis();
   }
