...
public class GuiInfoPopup extends AbstractGuiPopup<GuiInfoPopup> implements Typeable {
   ...
   private final GuiButton closeButton = (GuiButton)((GuiButton)((GuiButton)(new GuiButton()).setSize(150, 20)).onClick(() -> {
      this.close();
      this.onClosed.run();
   ...
   public GuiInfoPopup(GuiContainer container) {
      ...
      ((GuiPanel)this.popup.setLayout((new VerticalLayout()).setSpacing(10))).addElements(new VerticalLayout.Data(0.5D), new GuiElement[]{this.info, this.closeButton});
   }
