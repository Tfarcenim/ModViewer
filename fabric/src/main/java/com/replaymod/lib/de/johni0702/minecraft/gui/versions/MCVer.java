...
public class MCVer {
   ...
   public static void drawRect(int x, int y, int width, int height, ReadableColor tl, ReadableColor tr, ReadableColor bl, ReadableColor br) {
      ...
      vertexBuffer.vertex((double)x, (double)(y + height), 0.0D).color(bl.getRed(), bl.getGreen(), bl.getBlue(), bl.getAlpha()).endVertex();
      vertexBuffer.vertex((double)(x + width), (double)(y + height), 0.0D).color(br.getRed(), br.getGreen(), br.getBlue(), br.getAlpha()).endVertex();
      vertexBuffer.vertex((double)(x + width), (double)y, 0.0D).color(tr.getRed(), tr.getGreen(), tr.getBlue(), tr.getAlpha()).endVertex();
      vertexBuffer.vertex((double)x, (double)y, 0.0D).color(tl.getRed(), tl.getGreen(), tl.getBlue(), tl.getAlpha()).endVertex();
      tessellator.end();
   }
