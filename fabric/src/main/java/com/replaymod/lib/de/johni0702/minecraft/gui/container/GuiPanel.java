   ...
   public static class GuiPanelBuilder {
      ...
      public GuiPanel build() {
         ...
         case 0:
            ...
            break;
         case 1:
            withElements = Collections.singletonMap((GuiElement)this.withElements$key.get(0), (LayoutData)this.withElements$value.get(0));
            break;
         default:
            Map<GuiElement, LayoutData> withElements = new LinkedHashMap(this.withElements$key.size() < 1073741824 ? 1 + this.withElements$key.size() + (this.withElements$key.size() - 3) / 3 : Integer.MAX_VALUE);

            for(int $i = 0; $i < this.withElements$key.size(); ++$i) {
               withElements.put((GuiElement)this.withElements$key.get($i), (LayoutData)this.withElements$value.get($i));
            }
