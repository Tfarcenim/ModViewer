...
public abstract class AbstractGuiTextArea<T extends AbstractGuiTextArea<T>> extends AbstractGuiElement<T> implements Clickable, Typeable, Tickable, IGuiTextArea<T> {
   ...
   public T setText(String[] lines) {
      ...
      return (AbstractGuiTextArea)this.getThis();
   }
   ...
   public void draw(GuiRenderer renderer, ReadableDimension size, RenderInfo renderInfo) {
      ...
      if (this.hint != null && !this.isFocused() && Arrays.stream(this.text).allMatch(String::isEmpty)) {
         for(i = 0; i < maxLines && i < this.hint.length; ++i) {
            String line = fontRenderer.plainSubstrByWidth(this.hint[i], contentWidth);
            int posY = 4 + i * lineHeight;
            renderer.drawString(4, posY, this.textColorDisabled, line, true);
         }

      } else {
         for(i = 0; i < maxLines && i + this.currentYOffset < this.text.length; ++i) {
            ...
            if (lineY > fromY && lineY < toY) {
               ...
               leftStr = line.substring(Utils.clamp(fromX - leftTrimmed, 0, line.length()));
               renderer.invertColors(lineEnd, posY - 1 + lineHeight, lineEnd - fontRenderer.width(leftStr), posY - 1);
            } else if (lineY == toY) {
               leftStr = line.substring(0, Utils.clamp(toX - leftTrimmed, 0, line.length()));
               right = 4 + fontRenderer.width(leftStr);
               renderer.invertColors(right, posY - 1 + lineHeight, 4, posY - 1);
            }

            if (lineY == this.cursorY && this.blinkCursorTick / 6 % 2 == 0 && this.focused) {
               leftStr = line.substring(0, this.cursorX - leftTrimmed);
               right = 4 + fontRenderer.width(leftStr);
               if (this.cursorX == this.text[lineY].length()) {
                  renderer.drawString(right, posY, CURSOR_COLOR, "_", true);
               } else {
                  int var10002 = posY - 1;
                  Objects.requireNonNull(fontRenderer);
                  renderer.drawRect(right, var10002, 1, 1 + 9, CURSOR_COLOR);
               }
            }
         }
