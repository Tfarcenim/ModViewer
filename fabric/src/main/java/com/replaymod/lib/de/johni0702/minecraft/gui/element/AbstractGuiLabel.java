...
public abstract class AbstractGuiLabel<T extends AbstractGuiLabel<T>> extends AbstractGuiElement<T> implements IGuiLabel<T> {
   ...
   public void draw(GuiRenderer renderer, ReadableDimension size, RenderInfo renderInfo) {
      ...
      List<String> lines = (List)fontRenderer.getSplitter().splitLines(MCVer.literalText(this.text), size.getWidth(), Style.EMPTY).stream().map((it) -> {
   ...
   public T setText(String text) {
      this.text = text;
      return (AbstractGuiLabel)this.getThis();
   }
