...
public class HttpStatus {
   ...
   public static Buffer getResponseLine(int status) {
      return status >= __responseLine.length ? null : __responseLine[status];
   }

   static {
      Continue_BUFFER = CACHE.add("Continue", 100);
      Switching_Protocols_BUFFER = CACHE.add("Switching Protocols", 101);
      Processing_BUFFER = CACHE.add("Processing", 102);
      OK_BUFFER = CACHE.add("OK", 200);
      Created_BUFFER = CACHE.add("Created", 201);
      Accepted_BUFFER = CACHE.add("Accepted", 202);
      Non_Authoritative_Information_BUFFER = CACHE.add("Non Authoritative Information", 203);
      No_Content_BUFFER = CACHE.add("No Content", 204);
      Reset_Content_BUFFER = CACHE.add("Reset Content", 205);
      Partial_Content_BUFFER = CACHE.add("Partial Content", 206);
      Multi_Status_BUFFER = CACHE.add("Multi Status", 207);
      Multiple_Choices_BUFFER = CACHE.add("Multiple Choices", 300);
      Moved_Permanently_BUFFER = CACHE.add("Moved Permanently", 301);
      Moved_Temporarily_BUFFER = CACHE.add("Moved Temporarily", 302);
      Found_BUFFER = CACHE.add("Found", 302);
      See_Other_BUFFER = CACHE.add("See Other", 303);
      Not_Modified_BUFFER = CACHE.add("Not Modified", 304);
      Use_Proxy_BUFFER = CACHE.add("Use Proxy", 305);
      Bad_Request_BUFFER = CACHE.add("Bad Request", 400);
      Unauthorized_BUFFER = CACHE.add("Unauthorized", 401);
      Payment_Required_BUFFER = CACHE.add("Payment Required", 402);
      Forbidden_BUFFER = CACHE.add("Forbidden", 403);
      Not_Found_BUFFER = CACHE.add("Not Found", 404);
      Method_Not_Allowed_BUFFER = CACHE.add("Method Not Allowed", 405);
      Not_Acceptable_BUFFER = CACHE.add("Not Acceptable", 406);
      Proxy_Authentication_Required_BUFFER = CACHE.add("Proxy Authentication Required", 407);
      Request_Timeout_BUFFER = CACHE.add("Request Timeout", 408);
      Conflict_BUFFER = CACHE.add("Conflict", 409);
      Gone_BUFFER = CACHE.add("Gone", 410);
      Length_Required_BUFFER = CACHE.add("Length Required", 411);
      Precondition_Failed_BUFFER = CACHE.add("Precondition Failed", 412);
      Request_Entity_Too_Large_BUFFER = CACHE.add("Request Entity Too Large", 413);
      Request_URI_Too_Large_BUFFER = CACHE.add("Request URI Too Large", 414);
      Unsupported_Media_Type_BUFFER = CACHE.add("Unsupported Media Type", 415);
      Requested_Range_Not_Satisfiable_BUFFER = CACHE.add("Requested Range Not Satisfiable", 416);
      Expectation_Failed_BUFFER = CACHE.add("Expectation Failed", 417);
      Unprocessable_Entity_BUFFER = CACHE.add("Unprocessable Entity", 422);
      Locked_BUFFER = CACHE.add("Locked", 423);
      Failed_Dependency_BUFFER = CACHE.add("Failed Dependency", 424);
      Internal_Server_Error_BUFFER = CACHE.add("Internal Server Error", 500);
      Not_Implemented_BUFFER = CACHE.add("Not Implemented", 501);
      Bad_Gateway_BUFFER = CACHE.add("Bad Gateway", 502);
      Service_Unavailable_BUFFER = CACHE.add("Service Unavailable", 503);
      Gateway_Timeout_BUFFER = CACHE.add("Gateway Timeout", 504);
      HTTP_Version_Not_Supported_BUFFER = CACHE.add("HTTP Version Not Supported", 505);
      Insufficient_Storage_BUFFER = CACHE.add("Insufficient Storage", 507);
      Unknown_BUFFER = CACHE.add("Unknown", 999);
      ...
      for(int i = 0; i < __responseLine.length; ++i) {
         ...
         if (reason != null) {
            ...
            __responseLine[i] = new ByteArrayBuffer(bytes, 0, bytes.length, 0);
         }
      }
