...
import com.replaymod.lib.com.fasterxml.jackson.databind.deser.impl.BeanAsArrayBuilderDeserializer;
...
import com.replaymod.lib.com.fasterxml.jackson.databind.deser.impl.PropertyValueBuffer;
...
public class BuilderBasedDeserializer extends BeanDeserializerBase {
   ...
   private final Object vanillaDeserialize(JsonParser p, DeserializationContext ctxt, JsonToken t) throws IOException {
      Object bean;
      for(bean = this._valueInstantiator.createUsingDefault(ctxt); p.getCurrentToken() == JsonToken.FIELD_NAME; p.nextToken()) {
   ...
   protected Object deserializeWithUnwrapped(JsonParser p, DeserializationContext ctxt) throws IOException {
      if (this._delegateDeserializer != null) {
         ...
         for(Class activeView = this._needViewProcesing ? ctxt.getActiveView() : null; p.getCurrentToken() == JsonToken.FIELD_NAME; p.nextToken()) {
            String propName = p.getCurrentName();
            p.nextToken();
            SettableBeanProperty prop = this._beanProperties.find(propName);
            if (prop != null) {
