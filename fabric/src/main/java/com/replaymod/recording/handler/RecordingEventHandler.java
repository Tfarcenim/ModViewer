...
public class RecordingEventHandler extends EventRegistrations {
   ...
   public void unregister() {
      ...
      if (recordingEventSender.getRecordingEventHandler() == this) {
         recordingEventSender.setRecordingEventHandler((RecordingEventHandler)null);
      }
   ...
   public void spawnRecordingPlayer() {
      try {
         ...
         this.packetListener.save((Packet)(new ClientboundAddPlayerPacket(player)));
      ...
      if (this.mc.player != null) {
         ...
         try {
            ...
            if (!Objects.equals(rotationYawHead, this.rotationYawHeadBefore)) {
               this.packetListener.save((Packet)(new ClientboundRotateHeadPacket(player, (byte)rotationYawHead)));
            ...
            if (player.swinging && player.swingTime == 0) {
               this.packetListener.save((Packet)(new ClientboundAnimatePacket(player, player.swingingArm == InteractionHand.MAIN_HAND ? 0 : 3)));
            }
            ...
            for(int var15 = 0; var15 < vehicleId; ++var15) {
               ...
               if (!ItemStack.matches((ItemStack)this.playerItems.get(index), stack)) {
                  stack = stack != null ? stack.copy() : null;
                  this.playerItems.set(index, stack);
                  this.packetListener.save((Packet)(new ClientboundSetEquipmentPacket(player.getId(), Collections.singletonList(Pair.of(slot, stack)))));
               }
            }
            ...
            if (this.lastRiding != vehicleId) {
               ...
               this.packetListener.save((Packet)(new ClientboundSetEntityLinkPacket(player, vehicle)));
            }

            if (!player.isSleeping() && this.wasSleeping) {
               this.packetListener.save((Packet)(new ClientboundAnimatePacket(player, 2)));
               this.wasSleeping = false;
            }
