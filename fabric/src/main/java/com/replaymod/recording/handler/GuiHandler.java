...
public class GuiHandler extends EventRegistrations {
   ...
   private void onGuiInit(Screen gui) {
      if (gui instanceof SelectWorldScreen || gui instanceof JoinMultiplayerScreen) {
         ...
         final GuiCheckbox recordingCheckbox = (GuiCheckbox)((GuiCheckbox)(new GuiCheckbox()).setI18nLabel("replaymod.gui.settings.record" + (sp ? "singleplayer" : "server"), new Object[0])).setChecked((Boolean)settingsRegistry.get(setting));
      ...
      if (gui instanceof EditServerScreen) {
         ...
         final GuiButton replayButton = (GuiButton)(new GuiReplayButton()).onClick(() -> {
            ...
            GuiToggleButton<String> autoRecording = (GuiToggleButton)((GuiToggleButton)((GuiToggleButton)(new GuiToggleButton()).setI18nLabel("replaymod.gui.settings.autostartrecording", new Object[0])).setValues(new String[]{I18n.get("replaymod.gui.settings.default", new Object[0]), I18n.get("options.off", new Object[0]), I18n.get("options.on", new Object[0])})).setSelected(state == null ? 0 : (state ? 2 : 1));
            autoRecording.onClick(() -> {
               int selected = autoRecording.getSelected();
               serverInfoExt.setAutoRecording(selected == 0 ? null : selected == 2);
            });
            GuiInfoPopup.open(vanillaGui, (GuiElement[])(autoRecording));
         });
         ((GuiScreen)vanillaGui.setLayout(new CustomLayout<GuiScreen>(vanillaGui.getLayout()) {
            protected void layout(GuiScreen container, int width, int height) {
