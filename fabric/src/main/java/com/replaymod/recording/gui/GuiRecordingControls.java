   ...
   private void injectIntoIngameMenu(Screen guiScreen, Collection<AbstractWidget> buttonList) {
      if (guiScreen instanceof PauseScreen) {
         if (!buttonList.isEmpty()) {
            final Function<Integer, Integer> yPos = (Function)((Optional)MCVer.findButton(buttonList, "menu.returnToMenu", 1).map(Optional::of).orElse(MCVer.findButton(buttonList, "menu.disconnect", 1))).map((it) -> {
               return (height) -> {
                  return it.getY();
               };
            }).orElse((height) -> {
               return height / 4 + 120 - 16;
            });
