                        ...
                        while(true) {
                           ...
                           do {
                              if (!var11.hasNext()) {
                                 ...
                                 path.getKeyframes().stream().map((k) -> {
                                    return Pair.of(k, k.getValue(CameraProperties.POSITION).map(this::tripleD2Vec));
                                 }).filter((p) -> {
                                    return ((Optional)p.getRight()).isPresent();
                                 }).map((p) -> {
                                    return Pair.of((Keyframe)p.getLeft(), (Vector3f)((Optional)p.getRight()).get());
                                 }).filter((p) -> {
                                    return distanceSquared((Vector3f)p.getRight(), viewPos) < (double)renderDistanceSquared;
                                 }).sorted(new PathPreviewRenderer.KeyframeComparator(viewPos)).forEachOrdered((p) -> {
                                    this.drawPoint(viewPos, (Vector3f)p.getRight(), (Keyframe)p.getLeft());
                                 });
                                 GL11.glBlendFunc(770, 771);
                                 GL11.glEnable(2929);
                                 int time = guiPathing.timeline.getCursorPosition();
                                 Optional<Integer> entityId = path.getValue(SpectatorProperty.PROPERTY, (long)time);
                                 Optional replayTime;
                                 if (entityId.isPresent()) {
                                    if (entityTracker != null) {
                                       replayTime = timePath.getValue(TimestampProperty.PROPERTY, (long)time);
                                       if (replayTime.isPresent()) {
                                          Location loc = entityTracker.getEntityPositionAtTimestamp((Integer)entityId.get(), (long)(Integer)replayTime.get());
                                          if (loc != null) {
                                    ...
                                    replayTime = path.getValue(CameraProperties.POSITION, (long)time).map(this::tripleD2Vec);
                                    Optional<Vector3f> cameraRot = path.getValue(CameraProperties.ROTATION, (long)time).map(this::tripleF2Vec);
                                    if (replayTime.isPresent() && cameraRot.isPresent()) {
                                       this.drawCamera(viewPos, (Vector3f)replayTime.get(), (Vector3f)cameraRot.get());
                                       return;
                                    }
                                 }
                           ...
                           for(int i = 0; (long)i <= steps; ++i) {
                              ...
                              if (spectator) {
                                 ...
                                 entityId = path.getValue(CameraProperties.POSITION, time).map(this::tripleD2Vec);
                                 if (entityId.isPresent()) {
   ...
   private void drawConnection(Vector3f view, Vector3f pos1, Vector3f pos2, int color, int renderDistanceSquared) {
      if (!(distanceSquared(view, pos1) > (double)renderDistanceSquared)) {
         if (!(distanceSquared(view, pos2) > (double)renderDistanceSquared)) {
            ...
            MCVer.emitLine(buffer, Vector3f.sub(pos1, view, (Vector3f)null), Vector3f.sub(pos2, view, (Vector3f)null), color);
