...
public class SPTimeline implements PathingRegistry {
   ...
   public void addPositionKeyframe(long time, double posX, double posY, double posZ, float yaw, float pitch, float roll, int spectated) {
      ...
      Change change = CombinedChange.createFromApplied(change, updateChange);
      if (path.getSegments().size() == 1) {
         PathSegment segment = (PathSegment)path.getSegments().iterator().next();
   ...
   public void removePositionKeyframe(long time) {
      ...
      change.apply(this.timeline);
      Change change = CombinedChange.createFromApplied(change, this.updateInterpolators());
      Change specPosUpdate = this.updateSpectatorPositions();
      specPosUpdate.apply(this.timeline);
      change = CombinedChange.createFromApplied(change, specPosUpdate);
      this.timeline.pushChange(change);
   ...
   public void removeTimeKeyframe(long time) {
      ...
      Change specPosUpdate = this.updateSpectatorPositions();
      specPosUpdate.apply(this.timeline);
      Change change = CombinedChange.createFromApplied(change, specPosUpdate);
      this.timeline.pushChange(change);
   }
   ...
   public Change moveKeyframe(SPTimeline.SPPath spPath, long oldTime, long newTime) {
      ...
      if (Iterables.getLast(path.getKeyframes()) != newKf) {
         restoreInterpolatorChange = (Change)lostInterpolator.flatMap((interpolator) -> {
            return path.getSegments().stream().filter((s) -> {
               return s.getStartKeyframe() == newKf;
            }).findFirst().map((segment) -> {
         ...
         restoreInterpolatorChange = (Change)path.getSegments().stream().filter((s) -> {
            ...
            return lostInterpolator.map((interpolator) -> {
               return (Change)(newKf.getValue(ExplicitInterpolationProperty.PROPERTY).isPresent() ? CombinedChange.create(SetInterpolator.create(segment, interpolator), UpdateKeyframeProperties.create(path, segment.getStartKeyframe()).setValue(ExplicitInterpolationProperty.PROPERTY, ObjectUtils.NULL).done()) : SetInterpolator.create(segment, interpolator));
            });
         }).orElseGet(() -> {
            return CombinedChange.create();
         });
      ...
      if (spPath == SPTimeline.SPPath.POSITION) {
         ...
         if (path.getSegments().size() == 1) {
            ...
            interpolatorUpdateChange = SetInterpolator.create((PathSegment)path.getSegments().iterator().next(), (Interpolator)firstInterpolator.get());
         } else {
      ...
      return CombinedChange.createFromApplied(removeChange, addChange, propertyChange, restoreInterpolatorChange, (Change)interpolatorUpdateChange, spectatorChange);
   }
   ...
   private Change updateInterpolators() {
      ...
      while(var6.hasNext()) {
         ...
         if (this.isSpectatorSegment(segment)) {
            ...
            Interpolator currentInterpolator = (Interpolator)updates.getOrDefault(segment, segment.getInterpolator());
      ...
      if (this.entityTracker == null) {
         ...
         while(var2.hasNext()) {
            ...
            if (spectator.isPresent()) {
               ...
               if (time.isPresent()) {
                  Location expected = this.entityTracker.getEntityPositionAtTimestamp((Integer)spectator.get(), (long)(Integer)time.get());
                  if (expected != null) {
                     Triple<Double, Double, Double> pos = (Triple)keyframe.getValue(CameraProperties.POSITION).orElse(Triple.of(0.0D, 0.0D, 0.0D));
                     Triple<Float, Float, Float> rot = (Triple)keyframe.getValue(CameraProperties.ROTATION).orElse(Triple.of(0.0F, 0.0F, 0.0F));
                     Location actual = new Location((Double)pos.getLeft(), (Double)pos.getMiddle(), (Double)pos.getRight(), (Float)rot.getLeft(), (Float)rot.getRight());
   ...
   private Interpolator cloneInterpolator(Interpolator interpolator) {
      ...
      Collection var10000 = interpolator.getKeyframeProperties();
      Objects.requireNonNull(cloned);
      var10000.forEach(cloned::registerProperty);
      return cloned;
   }
