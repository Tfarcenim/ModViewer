...
public class GuiPathing {
   ...
   public GuiPathing(ReplayMod core, ReplayModSimplePathing mod, ReplayHandler replayHandler) {
      ...
      this.renderButton = (GuiButton)((GuiButton)((GuiButton)((GuiButton)((GuiButton)(new GuiButton()).onClick(new Runnable() {
         public void run() {
            GuiPathing.this.abortPathPlayback();
            GuiScreen screen = GuiRenderSettings.createBaseScreen();
            (new GuiRenderQueue(screen, GuiPathing.this.replayHandler, () -> {
               ...
               {
                  super(container, replayHandler, timelineSupplier);
                  this.this$1 = this$1;
               }
      ...
      })).setSize(20, 20)).setTexture(ReplayMod.TEXTURE, 256)).setSpriteUV(40, 0)).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.ingame.menu.renderpath", new Object[0]));
      ...
      this.timeline = (GuiKeyframeTimeline)((GuiKeyframeTimeline)(new GuiKeyframeTimeline(this) {
         public void draw(GuiRenderer renderer, ReadableDimension size, RenderInfo renderInfo) {
            if (GuiPathing.this.player.isActive()) {
               ((GuiKeyframeTimeline)this.setCursorPosition((int)GuiPathing.this.player.getTimePassed())).ensureCursorVisibleWithPadding();
            }
      ...
      ((GuiHorizontalScrollbar)this.scrollbar.onValueChanged(new Runnable() {
         public void run() {
            GuiPathing.this.timeline.setOffset((int)(GuiPathing.this.scrollbar.getPosition() * (double)GuiPathing.this.timeline.getLength()));
      ...
      this.zoomInButton = (GuiButton)((GuiButton)((GuiButton)((GuiButton)((GuiButton)(new GuiButton()).setSize(9, 9)).onClick(new Runnable() {
         public void run() {
            GuiPathing.this.zoomTimeline(0.6666666666666666D);
         }
      })).setTexture(ReplayMod.TEXTURE, 256)).setSpriteUV(40, 20)).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.ingame.menu.zoomin", new Object[0]));
      this.zoomOutButton = (GuiButton)((GuiButton)((GuiButton)((GuiButton)((GuiButton)(new GuiButton()).setSize(9, 9)).onClick(new Runnable() {
         public void run() {
            GuiPathing.this.zoomTimeline(1.5D);
         }
      })).setTexture(ReplayMod.TEXTURE, 256)).setSpriteUV(40, 30)).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.ingame.menu.zoomout", new Object[0]));
      this.zoomButtonPanel = (GuiPanel)((GuiPanel)(new GuiPanel()).setLayout((new VerticalLayout(VerticalLayout.Alignment.CENTER)).setSpacing(2))).addElements((LayoutData)null, new GuiElement[]{this.zoomInButton, this.zoomOutButton});
      this.timelinePanel = (GuiPanel)((GuiPanel)((GuiPanel)(new GuiPanel()).setSize(Integer.MAX_VALUE, 40)).setLayout(new CustomLayout<GuiPanel>() {
         protected void layout(GuiPanel container, int width, int height) {
            this.pos(GuiPathing.this.zoomButtonPanel, width - this.width(GuiPathing.this.zoomButtonPanel), 10);
            this.pos(GuiPathing.this.timelineTime, 0, 2);
            this.size(GuiPathing.this.timelineTime, this.x(GuiPathing.this.zoomButtonPanel), 8);
            this.pos(GuiPathing.this.timeline, 0, this.y(GuiPathing.this.timelineTime) + this.height(GuiPathing.this.timelineTime));
            this.size(GuiPathing.this.timeline, this.x(GuiPathing.this.zoomButtonPanel) - 2, 20);
            this.pos(GuiPathing.this.scrollbar, 0, this.y(GuiPathing.this.timeline) + this.height(GuiPathing.this.timeline) + 1);
            this.size(GuiPathing.this.scrollbar, this.x(GuiPathing.this.zoomButtonPanel) - 2, 9);
         }
      })).addElements((LayoutData)null, new GuiElement[]{this.timeline, this.timelineTime, this.scrollbar, this.zoomButtonPanel});
      this.panel = (GuiPanel)((GuiPanel)(new GuiPanel()).setLayout((new HorizontalLayout(HorizontalLayout.Alignment.CENTER)).setSpacing(5))).addElements(new HorizontalLayout.Data(0.5D), new GuiElement[]{this.playPauseButton, this.renderButton, this.positionKeyframeButton, this.timeKeyframeButton, this.timelinePanel});
      this.prevSpeed = -1;
      this.prevTime = -1;
      this.core = core;
      this.mod = mod;
      ...
      ((GuiButton)this.playPauseButton.setSpriteUV(new ReadablePoint() {
         ...
         public void run() {
            if (GuiPathing.this.player.isActive()) {
               ...
               boolean ignoreTimeKeyframes = MCVer.Keyboard.isKeyDown(340);
               Timeline timeline = (Timeline)GuiPathing.this.preparePathsForPlayback(ignoreTimeKeyframes).okOrElse((err) -> {
                  GuiInfoPopup.open(GuiPathing.this.overlay, (String[])err);
                  return null;
               });
               if (timeline == null) {
                  return;
               ...
               int startTime = MCVer.Keyboard.isKeyDown(341) ? 0 : GuiPathing.this.timeline.getCursorPosition();
               ListenableFuture<Void> future = GuiPathing.this.player.start(timeline, (long)startTime);
               ...
               Futures.addCallback(future, new FutureCallback<Void>(future) {
                  // $FF: synthetic field
                  ...
                  public void onSuccess(@Nullable Void result) {
                     if (this.val$future.isCancelled()) {
                        this.this$1.val$core.printInfoToChat("replaymod.chat.pathinterrupted");
                     } else {
                        this.this$1.val$core.printInfoToChat("replaymod.chat.pathfinished");
                     }

                     this.this$1.this$0.overlay.setCloseable(true);
                  }
      ...
      ((GuiButton)this.positionKeyframeButton.setSpriteUV(new ReadablePoint() {
         public int getX() {
            ...
            if (keyframePath != SPTimeline.SPPath.POSITION) {
               ...
               keyframePath = mod.getCurrentTimeline().isPositionKeyframe(keyframeTime) ? SPTimeline.SPPath.POSITION : null;
            }
         ...
         public int getY() {
            SPTimeline.SPPath keyframePath = mod.getSelectedPath();
            if (keyframePath != SPTimeline.SPPath.POSITION) {
               keyframePath = mod.getCurrentTimeline().isPositionKeyframe((long)GuiPathing.this.timeline.getCursorPosition()) ? SPTimeline.SPPath.POSITION : null;
            }
      ...
      ((GuiButton)this.timeKeyframeButton.setSpriteUV(new ReadablePoint() {
         ...
         public int getY() {
            ...
            if (keyframePath != SPTimeline.SPPath.TIME) {
               keyframePath = mod.getCurrentTimeline().isTimeKeyframe((long)GuiPathing.this.timeline.getCursorPosition()) ? SPTimeline.SPPath.TIME : null;
            }
      ...
      this.overlay.setLayout(new CustomLayout<GuiReplayOverlay>(this.overlay.getLayout()) {
         protected void layout(GuiReplayOverlay container, int width, int height) {
            ...
            this.pos(GuiPathing.this.panel, 10, this.y(GuiPathing.this.overlay.topPanel) + this.height(GuiPathing.this.overlay.topPanel) + 3);
   ...
   public void keyframeRepoButtonPressed() {
      ...
      try {
         TimelineSerialization serialization = new TimelineSerialization(this.mod.getCurrentTimeline(), (ReplayFile)null);
   ...
   public void clearKeyframesButtonPressed() {
      GuiYesNoPopup.open(this.replayHandler.getOverlay(), ((GuiLabel)(new GuiLabel()).setI18nText("replaymod.gui.clearcallback.title", new Object[0])).setColor(Colors.BLACK)).setYesI18nLabel("gui.yes").setNoI18nLabel("gui.no").onAccept(() -> {
   ...
   private Integer computeSyncTime(int cursor) {
      int time = this.replayHandler.getReplaySender().currentTimeStamp();
      Keyframe keyframe = (Keyframe)this.mod.getCurrentTimeline().getTimePath().getKeyframes().stream().filter((it) -> {
      ...
      if (this.loadEntityTracker(() -> {
         ...
         case POSITION:
            if (this.mod.getSelectedPath() == path) {
               ...
               timeline.addPositionKeyframe((long)time, camera.getX(), camera.getY(), camera.getZ(), camera.getYRot(), camera.getXRot(), camera.roll, spectatedId);
               this.mod.setSelected(path, (long)time);
            }
         }
   ...
   private class LoadEntityTrackerPopup extends AbstractGuiPopup<GuiPathing.LoadEntityTrackerPopup> {
      ...
      public LoadEntityTrackerPopup(GuiContainer container) {
         super(container);
         this.progressBar = (GuiProgressBar)((GuiProgressBar)(new GuiProgressBar(this.popup)).setSize(300, 20)).setI18nLabel("replaymod.gui.loadentitytracker", new Object[0]);
         this.open();
      }
