...
public class KeyBindingRegistry extends EventRegistrations {
   ...
   private boolean handleRaw(int keyCode, int action) {
      if (action != 1) {
         ...
         Iterator var3 = this.rawHandlers.get(keyCode).iterator();

         while(var3.hasNext()) {
            Supplier handler = (Supplier)var3.next();

            try {
               if ((Boolean)handler.get()) {
                  return true;
               }
            } catch (Throwable var8) {
               CrashReport crashReport = CrashReport.forThrowable(var8, "Handling Raw Key Binding");
               CrashReportCategory category = crashReport.addCategory("Key Binding");
               category.setDetail("Key Code", () -> {
                  return keyCode.makeConcatWithConstants<invokedynamic>(keyCode);
               });
